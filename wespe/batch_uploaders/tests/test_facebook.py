...
class TestFacebookBatch(TestCase):
    ...
    def test_execute_calls_initialize_execution_retrial_conditions(self, mock_initialize_execution_retrial_conditions):
        self.batch.execute()
    ...
    def test_execute_calls_add_request_on_batch_ignores_requests_that_have_a_response_set_already(self):
    ...
    @patch('wespe.batch_uploaders.facebook.should_retry_facebook_batch', return_value=True)
    def test_execute_raises_RetryError_when_should_retry_is_True_for_all_attempts(self, mock_should_retry):
        with self.assertRaises(RetryError):
            self.batch.execute()

    @patch('wespe.batch_uploaders.facebook.should_retry_facebook_batch', return_value=True)
    def test_execute_retries_for_max_attempts_when_should_retry_is_True_for_all_of_them(self, mock_should_retry):
        try:
            self.batch.execute()
        except RetryError:
            pass

        self.assertEqual(self.batch.execute.retry.statistics.get('attempt_number'), FacebookBatch.MAX_ATTEMPTS)

    @patch('wespe.batch_uploaders.facebook.should_retry_facebook_batch', return_value=False)
    ...
    def test_initialize_execution_retrial_conditions_sets_the_right_retrial_arguments_for_execution(self):
        ...
        with self.subTest('sets the maximum number of attempts'):
            self.assertEqual(retry_settings.stop.max_attempt_number, self.batch.MAX_ATTEMPTS)

        with self.subTest('sets the exponential multiplier'):
            self.assertEqual(retry_settings.wait.multiplier, self.batch.WAIT_EXPONENTIAL_MULTIPLIER)

        with self.subTest('sets the exponential minimum threshold'):
            self.assertEqual(retry_settings.wait.min, self.batch.WAIT_EXPONENTIAL_MIN_IN_SECONDS)

        with self.subTest('sets the exponential maximum threshold'):
            self.assertEqual(retry_settings.wait.max, self.batch.WAIT_EXPONENTIAL_MAX_IN_SECONDS)
...
class TestFacebookBatchUploader(TestCase):
    ...
    def test_execute_ignores_RetryError_exceptions_raised_by_individual_batches(self, mock_execute):
        self.batch_uploader.execute()
